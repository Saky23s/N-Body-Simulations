...
template <unsigned int blockSize>
__global__ void calculate_and_reduce_acceleration(double* d_masses, double* d_position, double* d_acceleration, int n, unsigned int number_of_blocks_j);

__global__ void cuda_leapfrog(double* d_position, double* d_velocity, double* d_acceleration, int n, double ddt, double half_dt, int both);

template <unsigned int blockSize>
__device__ void full_block_reduction (double* sdata, double* d_acceleration, int n, unsigned int number_of_blocks_j);
...
{       
    ...
    cudaMemcpy( simulation->d_velocity,  simulation->velocity, simulation->n * 3 * sizeof(simulation->velocity[0]),cudaMemcpyHostToDevice);
    ...
    {   
        case 1024:
            calculate_and_reduce_acceleration<1024><<<simulation->gridDims, simulation->threadBlockDims, 3 * simulation->threadBlockDims.x * simulation->threadBlockDims.y * sizeof(double)>>>(simulation->d_masses, simulation->d_position, simulation->d_acceleration, simulation->n, simulation->gridDims.y);
            cuda_leapfrog<<<simulation->gridDims, simulation->threadBlockDims>>>(simulation->d_position, simulation->d_velocity, simulation->d_acceleration, simulation->n, dt, simulation->half_dt, 1);
            break;
        case 512:
            calculate_and_reduce_acceleration<512><<<simulation->gridDims, simulation->threadBlockDims, 3 * simulation->threadBlockDims.x * simulation->threadBlockDims.y * sizeof(double)>>>(simulation->d_masses, simulation->d_position, simulation->d_acceleration, simulation->n, simulation->gridDims.y);
            cuda_leapfrog<<<simulation->gridDims, simulation->threadBlockDims>>>(simulation->d_position, simulation->d_velocity, simulation->d_acceleration, simulation->n, dt, simulation->half_dt, 1);
            break;
        case 256:
            calculate_and_reduce_acceleration<256><<<simulation->gridDims, simulation->threadBlockDims, 3 * simulation->threadBlockDims.x * simulation->threadBlockDims.y * sizeof(double)>>>(simulation->d_masses, simulation->d_position, simulation->d_acceleration, simulation->n, simulation->gridDims.y);
            cuda_leapfrog<<<simulation->gridDims, simulation->threadBlockDims>>>(simulation->d_position, simulation->d_velocity, simulation->d_acceleration, simulation->n, dt, simulation->half_dt, 1);
            break;
        case 128:
            calculate_and_reduce_acceleration<128><<<simulation->gridDims, simulation->threadBlockDims, 3 * simulation->threadBlockDims.x * simulation->threadBlockDims.y * sizeof(double)>>>(simulation->d_masses, simulation->d_position, simulation->d_acceleration, simulation->n, simulation->gridDims.y);
            cuda_leapfrog<<<simulation->gridDims, simulation->threadBlockDims>>>(simulation->d_position, simulation->d_velocity, simulation->d_acceleration, simulation->n, dt, simulation->half_dt, 1);
            break;
        case 64:
            calculate_and_reduce_acceleration<64><<<simulation->gridDims, simulation->threadBlockDims, 3 * simulation->threadBlockDims.x * simulation->threadBlockDims.y * sizeof(double)>>>(simulation->d_masses, simulation->d_position, simulation->d_acceleration, simulation->n, simulation->gridDims.y);
            cuda_leapfrog<<<simulation->gridDims, simulation->threadBlockDims>>>(simulation->d_position, simulation->d_velocity, simulation->d_acceleration, simulation->n, dt, simulation->half_dt, 1);
            break;            
        case 32:
            calculate_and_reduce_acceleration<32><<<simulation->gridDims, simulation->threadBlockDims, 3 * simulation->threadBlockDims.x * simulation->threadBlockDims.y * sizeof(double)>>>(simulation->d_masses, simulation->d_position, simulation->d_acceleration, simulation->n, simulation->gridDims.y);
            cuda_leapfrog<<<simulation->gridDims, simulation->threadBlockDims>>>(simulation->d_position, simulation->d_velocity, simulation->d_acceleration, simulation->n, dt, simulation->half_dt, 1);
            break;
    }
    ...
    {   
        case 1024:
            calculate_and_reduce_acceleration<1024><<<simulation->gridDims, simulation->threadBlockDims, 3 * simulation->threadBlockDims.x * simulation->threadBlockDims.y * sizeof(double)>>>(simulation->d_masses, simulation->d_position, simulation->d_acceleration, simulation->n, simulation->gridDims.y);
            cuda_leapfrog<<<simulation->gridDims, simulation->threadBlockDims>>>(simulation->d_position, simulation->d_velocity, simulation->d_acceleration, simulation->n, dt, simulation->half_dt, 0);
            break;
        case 512:
            calculate_and_reduce_acceleration<512><<<simulation->gridDims, simulation->threadBlockDims, 3 * simulation->threadBlockDims.x * simulation->threadBlockDims.y * sizeof(double)>>>(simulation->d_masses, simulation->d_position, simulation->d_acceleration, simulation->n, simulation->gridDims.y);
            cuda_leapfrog<<<simulation->gridDims, simulation->threadBlockDims>>>(simulation->d_position, simulation->d_velocity, simulation->d_acceleration, simulation->n, dt, simulation->half_dt, 0);
    ...
    cudaMemcpy( simulation->velocity, simulation->d_velocity, 3 * simulation->n  * sizeof(simulation->d_velocity[0]), cudaMemcpyDeviceToHost);
...
__global__ void cuda_leapfrog(double* d_position, double* d_velocity, double* d_acceleration, int n, double ddt, double half_dt, int both)
{
    int offset = (threadIdx.y + blockIdx.y * blockDim.y) * 3;
    ...
    {   
        ...
        d_velocity[offset] = d_velocity[offset] + d_acceleration[offset] * half_dt;
        d_velocity[offset+1] = d_velocity[offset+1] + d_acceleration[offset+1] * half_dt;
        d_velocity[offset+2] = d_velocity[offset+2] + d_acceleration[offset+2] * half_dt;

        //Use that velocity to full step the position if required
        //  rn+1 = rn + dt*vn+1/2
        if( both == 1)
        {
            d_position[offset] = d_position[offset] + d_velocity[offset] * ddt;
            d_position[offset+1] = d_position[offset+1] + d_velocity[offset+1] * ddt;
            d_position[offset+2] = d_position[offset+2] + d_velocity[offset+2] * ddt;
        }
    }
}
...
{   
    ...
    full_block_reduction<blockSize>(sdata, d_acceleration, n, number_of_blocks_j);
}
 ...
 */
{
    ...
    if (y == 0)
    {
        d_acceleration[ioffset] += S(blockDim.x, blockDim.y, 0, x, 0, sdata);
        d_acceleration[ioffset+1] += S(blockDim.x, blockDim.y, 1, x, 0, sdata);
        d_acceleration[ioffset+2] += S(blockDim.x, blockDim.y, 2, x, 0, sdata);
    } 
}
...
{   
    ...
    status = cudaMalloc(&simulation->d_velocity, simulation->n  * 3 * sizeof(simulation->d_velocity[0]));
    cudaErrorCheck(status);

    status = cudaMalloc(&simulation->d_acceleration, simulation->n * 3 * sizeof(simulation->d_acceleration[0]));
    cudaErrorCheck(status);
