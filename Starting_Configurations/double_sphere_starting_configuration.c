...
{   
    ...
    FILE* position_file = fopen("../Starting_Configurations/bin_files/double_sphere.bin", "wb");
    ...
    double scalar = sqrt((n/2) / (M_PI * 2));
    for( int i = 0; i <  floor(n/2.0); i ++)
    {
        double y = 1 - (i / (double) (n/2 - 1)) * 2 ;
    ...
    for( int i = 0; i <  ceil(n/2.0); i ++)
    {
        ...
        printf("%d: %lf,%lf,%lf\n",i+(int)floor(n/2.0),x*scalar,y*scalar,z*scalar);
